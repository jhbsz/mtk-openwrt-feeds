...
function mt7620_up(devname)
	local nixio = require("nixio")
	local mtkwifi = require("mtkwifi")
	nixio.syslog("debug", "mt7620_up called!")
	...
	os.execute(" rm -rf /tmp/mtk/wifi/mt7620*.need_reload")
end

function mt7620_down(devname)
	local nixio = require("nixio")
	local mtkwifi = require("mtkwifi")
	nixio.syslog("debug", "mt7620_down called!")
...
function mt7620_reload(devname)
	local nixio = require("nixio")
...
function mt7620_reset(devname)
	local nixio = require("nixio")
	local mtkwifi = require("mtkwifi")
	nixio.syslog("debug", "mt7620_reset called!")
	if mtkwifi.exists("/rom/etc/wireless/mt7620/") then
		os.execute("rm -rf /etc/wireless/mt7620/")
		os.execute("cp -rf /rom/etc/wireless/mt7620/ /etc/wireless/")
		mt7620_reload()
	else
...
function mt7620_status(devname)
	return wifi_common_status()
end

function mt7620_detect(devname)
	local nixio = require("nixio")
	local mtkwifi = require("mtkwifi")
	nixio.syslog("debug", "mt7620_detect called!")
